...
**`form-input`** + **`form-input-focused`**

- Default: background `transparent`, text `{colors.ink}`, type `{typography.body-md}`, rounded `{rounded.sm}` (8px/18px depending on scale), padding `16px`, height `48px`. Border is 1px solid `{colors.hairline}`.
- Focused: background stays `transparent`, border transforms to 2px solid `{colors.ink}`, no halo or drop shadow. This crisp black bounding box acts as the universal focus state for podium-cds forms.
